...
impl SessionContext {
    ...
    async fn drop_schema(&self, cmd: DropCatalogSchema) -> Result<DataFrame> {
        ...
        let state = self.state.read();
        let default_catalog = &state.config_options().catalog.default_catalog;
        let schema_ref = name.clone().into();
        let (catalog_name, schema_name) = match schema_ref {
            SchemaReference::Bare { schema } => {
                (std::borrow::Cow::from(default_catalog), schema)
            }
            SchemaReference::Full { schema, catalog } => (catalog, schema),
        };
        if let Some(catalog) = state.catalog_list.catalog(&catalog_name) {
            let dereg = catalog.deregister_schema(&schema_name, cascade)?;
            ...
            self.schema_doesnt_exist_err(name)
        }
    }
